...
void generateIntFile(const char *filename) {
  int count = 1000000;
  int i;
  uint *int_array = (uint *)malloc(sizeof(uint) * count);
  for (i = 0; i < count; i++) {
    int_array[i] = i % 120;
  }
  FILE *file = fopen(filename, "wb");
  if (!file)  exit(-1);
  fwrite((char *)int_array, sizeof(uint), count, file);
  fclose(file);
}
